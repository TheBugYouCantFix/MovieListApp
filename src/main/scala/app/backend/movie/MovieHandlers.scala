...
object MovieHandlers:
  ...
  def getMovieByIdHandler(id: MovieId): ZIO[MovieRepo, domain.Error, Movie] = 
  ...
  def updateMovieHandler(id: MovieId, movie: Movie): ZIO[MovieRepo, domain.Error, Unit] =
    ZIO.serviceWithZIO[MovieRepo](_.updateTo(id, movie)).mapError(e => MovieError(e.getMessage))

  def removeMovieByIdHandler(id: MovieId): ZIO[MovieRepo, domain.Error, Unit] =
